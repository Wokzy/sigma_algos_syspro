...
private:
	vector<bool> filter;
	size_t k; // amount of hash functions
	size_t n; // amount of data
	size_t m; // filter size (in bits)

	size_t ip_hash(unsigned char addr[4], size_t crc) {
...
public:
	...
	void insert(unsigned char addr[4]) {
		for (size_t i = 0; i < k; i++)
			filter[ip_hash(addr, i)] = true;
	}

	bool contains(unsigned char addr[4]) {
		for (size_t i = 0; i < k; i++)
			if (!filter[ip_hash(addr, i)])
				return false;
		return true;
	}
};
